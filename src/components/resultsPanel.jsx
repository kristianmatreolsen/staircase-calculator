  ...
  return (
    <div style={{ padding: "13px 15px" }}>
      ...
      {result.recommendations?.length > 0 && (
        <div style={{ marginTop: 13, padding: "10px 11px", background: "#0C1E10", border: "1px solid #1A4020", borderRadius: 6 }}>
          <div style={{ fontSize: 9, fontFamily: FONTS.mono, color: "#44A050", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 8 }}>
            💡 Recommendations
          </div>
          {result.recommendations.map((rec, i) => (
            <div key={i} style={{ display: "flex", gap: 6, marginBottom: 6, alignItems: "flex-start" }}>
              <span style={{ fontSize: 9, color: CC.warn, flexShrink: 0, marginTop: 1 }}>→</span>
              <div style={{ fontSize: 9.5, fontFamily: FONTS.mono, color: "#C07830", lineHeight: 1.65 }}>{rec}</div>
            </div>
          ))}
        </div>
      )}
